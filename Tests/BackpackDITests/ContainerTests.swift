...
struct Test {
    ...
    func testArguments() throws {
        ...
        sut.register(TestDependencyArgs.self) { _, name, age in
            return TestDependencyArgs(name: name, age: age)
        }

        let dependency = sut.resolve(TestDependencyArgs.self, arguments: name, age)

        #expect(dependency.name == name)
        #expect(dependency.age == age)
